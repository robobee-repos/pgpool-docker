...
if [[ -z "$PG_PRIMARY_USER" ]]; then
  PG_PRIMARY_USER="$PG_USERNAME"
fi

if [[ -z "$PG_PRIMARY_PASSWORD" ]]; then
  PG_PRIMARY_PASSWORD="$PG_PASSWORD"
fi

# create pcp.conf for PCP commands.
echo "${PG_PRIMARY_USER}:`/bin/pg_md5 $PG_PRIMARY_PASSWORD`" > $CONFIGS/pcp.conf
